<?php
//TODO: move the html to a view and clean up this class
...
class BreadCrumbs
{
    public function output()
    {
        ...
        if ( is_front_page() || 'llms_my_certificate' === get_post_type() || 'llms_certificate' === get_post_type() ) {
            return;
        }
        ...
        $args = [];
        $defaults  = array(
            'separator_icon'      => get_option('launchpad_settings_breadcrumbs_separator_icon', '>'),
            'breadcrumbs_id'      => 'breadcrumbs',
            'home_title'          => get_option('launchpad_settings_breadcrumbs_home_title', 'Home'),
        );

        $args      = apply_filters( 'launchpad_breadcrumbs_args', wp_parse_args( $args, $defaults ) );

        $separator = '<span class="separator"> ' . esc_attr( $args['separator_icon'] ) . ' </span>';

        /***** Begin Markup *****/
        // Open the breadcrumbs
        $html = '<div id="' . esc_attr( $args['breadcrumbs_id'] ) . '">';
        ...
        if ( is_singular( 'post' ) ) {
            ...
            if ( $post->post_parent ) {
                ...
                foreach ( $parents as $parent ) {
                    $html .= '<span class="item-parent item-parent-' . esc_attr( $parent ) . '"><a class="bread-link bread-parent bread-parent-' . esc_attr( $parent ) . '" href="' . esc_url( get_permalink( $parent ) ) . '" title="' . get_the_title( $parent ) . '">' . get_the_title( $parent ) . '</a></span>';
                    $html .= $separator;
                }
            }
            ...
            $html .= '<span class="item-cat item-custom-post-type-' . esc_attr( $post_type ) . '"><a class="bread-link bread-cat bread-custom-post-type-' . esc_attr( $post_type ) . '" href="' . esc_url( $post_type_archive ) . '" title="' . esc_attr( $post_type_object->labels->name ) . '">' . esc_attr( $post_type_object->labels->name ) . '</a></span>';
            $html .= $separator;
            ...
            if ( $parent !== 0 ) {
                ...
                $html .= '<span class="item-parent item-parent-' . esc_attr( $parent_category->slug ) . '"><a class="bread-link bread-parent bread-parent-' . esc_attr( $parent_category->slug ) . '" href="' . esc_url( $category_link ) . '" title="' . esc_attr( $parent_category->name ) . '">' . esc_attr( $parent_category->name ) . '</a></span>';
                $html .= $separator;
            }
            ...
            $html .= '<span class="item-current">' . get_the_title( get_option( 'page_for_posts' ) ) . '</span>';
        }
        $html .= '</div>';
        $html = apply_filters( 'launchpad_breadcrumbs_filter', $html );
        return wp_kses_post( $html );
    }

    public function breadcrumbs_enabled()
    {
        return strcmp(get_option('launchpad_settings_breadcrumbs_enable', false), 'yes') === 0 ? true : false;
    }
